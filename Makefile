...
# 架构映射
RPM_ARCH=$(shell if [ "$(ARCH)" = "amd64" ]; then echo "x86_64"; elif [ "$(ARCH)" = "arm64" ]; then echo "aarch64"; else echo "$(ARCH)"; fi)
...
build-deb: build
	...
	# 构建 deb 包
	cd $(BUILD_DIR) && dpkg-deb --build deb "$(BINARY_NAME)_$(PACKAGE_VERSION)_$(ARCH:=).deb"

.PHONY: build-rpm
build-rpm: build
	@echo "Building RPM package for $(ARCH) (RPM_ARCH=$(RPM_ARCH))..."
	mkdir -p $(RPM_DIR)/{BUILD,RPMS,SOURCES,SPECS,SRPMS}
	mkdir -p $(RPM_DIR)/BUILDROOT/$(BINARY_NAME)-$(PACKAGE_VERSION)-$(PACKAGE_RELEASE).$(RPM_ARCH)/{usr/local/bin,etc/auto-tunnel,etc/systemd/system,var/log/auto-tunnel}
	...
	echo "BuildRoot: %{_topdir}/BUILDROOT/%{name}-%{version}-%{release}.%{_arch}" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	...
	echo "%define _target_platform %{_arch}-redhat-linux" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "%define _target_cpu %{_arch}" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	...
	echo "" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "%build" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "# Nothing to build" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	...
	echo "install -D -m 755 %{_sourcedir}/$(BINARY_NAME) %{buildroot}/usr/local/bin/$(BINARY_NAME)" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "install -D -m 644 %{_sourcedir}/config.yaml %{buildroot}/etc/auto-tunnel/config.yaml" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "install -D -m 644 %{_sourcedir}/auto-tunnel.service %{buildroot}/etc/systemd/system/auto-tunnel.service" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	echo "" >> $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
	...
	# 构建 rpm 包
	rpmbuild -bb --target $(RPM_ARCH)-linux --define "_topdir $(PWD)/$(RPM_DIR)" $(RPM_DIR)/SPECS/$(BINARY_NAME).spec
